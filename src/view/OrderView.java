...
public class OrderView {
        ...
        do {
            ...
            switch (choose) {
                case 1:
                    ...
                    break;
                case 2: handleCreateInvoice(sc, orderService); break;
                case 3: handleUpdateInvoice(sc, orderService); break;
                case 4: handleDeleteInvoice(sc, orderService); break;
                case 5:
                    ...
                    if (!found) System.out.println("Invoice not found!");
                    break;
                case 6:
    ...
    private static void handleCreateInvoice(Scanner sc, OrderService service) {
        ...
        while (true) {
            ...
            } catch (Exception e) { System.out.println("Error displaying storage list!"); }

            if (availableCount == 0) { System.out.println("No tea products available!"); break; }
            ...
            if (matchedCatalog == null || !matchedCatalog[1].equalsIgnoreCase("Tea")) {
                System.out.println("Invalid ID!"); continue;
            }
    ...
    private static void printInvoiceTable(Invoice invoice, double serviceFee) {
        ...
        for (OrderItem item : invoice.getItemList()) {
            ...
            if (rawName.contains("|")) { String[] parts = rawName.split("\\|"); pId = parts[0]; pName = parts[1]; }
    ...
    private static void handleUpdateInvoice(Scanner sc, OrderService service) {
        ...
        if (!service.isExist(id)) { System.out.println("Invoice not found!"); return; }
        System.out.print("Enter New Employee ID: ");
        String newEmpId = sc.nextLine().trim();
        String matchedName = service.findEmployeeNameById(newEmpId);
        if(matchedName == null) { System.out.println("Employee ID does not exist!"); return; }
    ...
    private static void handleDeleteInvoice(Scanner sc, OrderService service) {
        ...
        String id = sc.nextLine().trim();
        if (service.deleteInvoice(id)) System.out.println("Deleted successfully!");
        else System.out.println("Invoice ID does not exist!");
    }
}