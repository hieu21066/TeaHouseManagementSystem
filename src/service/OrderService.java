...
public class OrderService {
    ...
    public OrderService() { invoiceList = OrderFile.load(); }
    ...
    public String generateNextInvoiceId() { return String.format("OD%03d", invoiceList.size() + 1); }
    ...
    public String findEmployeeNameById(String empId) {
        try (BufferedReader br = new BufferedReader(new FileReader("Employee.txt"))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (data[0].equalsIgnoreCase(empId)) return data[2].trim();
            }
        } catch (IOException e) { System.out.println("File reading error Employee.txt"); }
        return null;
    }
    ...
    public String[] findCatalogProduct(String prodId) {
        ...
        } catch (IOException e) { System.out.println("File reading error ProductCatalog.txt"); }
        return null;
    }
    ...
    public int getStorageQuantityInGrams(String prodId) {
        ...
        } catch (IOException e) { System.out.println("File reading error ProductStorage.txt"); }
        return 0;
    }
    ...
    public boolean takeOrderItem(Invoice invoice, String prodId, int amountInput) {
        ...
        if (currentGrams < amountInput) return false;
        ...
        if (!isDuplicate) invoice.addItem(compositeName, finalPriceEachGram, amountInput);
    ...
    private void updateStorageFile(String prodId, int amountBought) {
        ...
        } catch (IOException e) { System.out.println("Inventory update error!"); }
        ...
        } catch (IOException e) { System.out.println("Unable to save archive file!"); }
    }

    public boolean addInvoice(Invoice invoice) { invoiceList.add(invoice); OrderFile.save(invoiceList); return true; }
    public boolean isExist(String id) { return invoiceList.stream().anyMatch(i -> i.getInvoiceId().equalsIgnoreCase(id)); }
    public boolean updateInvoice(Invoice newInvoice) {
        for (Invoice old : invoiceList) {
            if (old.getInvoiceId().equalsIgnoreCase(newInvoice.getInvoiceId())) {
                ...
                OrderFile.save(invoiceList); return true;
            }
        }
        return false;
    }
    public boolean deleteInvoice(String id) {
        ...
        if (removed) OrderFile.save(invoiceList);
        return removed;
    }
